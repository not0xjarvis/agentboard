...
app.post('/api/projects', (req, res) => {
  const { name, description, status, category, priority, repo_url } = req.body;
  if (!name) return res.status(400).json({ error: 'name required' });
  try {
    const result = db.prepare(
      `INSERT INTO projects (name, description, status, category, priority, repo_url)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).run(name, description || '', status || 'Active', category || '', priority || 'P2', repo_url || '');
...
app.put('/api/projects/:id', (req, res) => {
  const fields = ['name', 'description', 'status', 'category', 'priority', 'repo_url'];
  const updates = [];
  const values = [];
...
app.post('/api/tasks', (req, res) => {
  ...
  if (!name) return res.status(400).json({ error: 'name required' });
  const result = db.prepare(
    `INSERT INTO tasks (name, project_id, description, status, priority, assignee, labels, size, due_date)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
...
app.get('/api/backlog', (req, res) => {
  const tasks = db.prepare(
    `SELECT t.*, p.name as project_name FROM tasks t
     ...
     WHERE t.status IN ('Backlog', 'Todo') AND t.assignee IN ('Agent', 'Unassigned')
...
app.post('/api/tasks/:id/claim', (req, res) => {
  ...
  db.prepare(
    "UPDATE tasks SET status = 'In Progress', assignee = ?, updated_at = datetime('now') WHERE id = ?"
  ...
  if (task) {
    db.prepare('INSERT INTO comments (task_id, author, content) VALUES (?, ?, ?)').run(
      task.id, assignee || 'Agent', `Claimed task and started work`
    );
  }
  task ? res.json(task) : res.status(404).json({ error: 'not found' });
});
